...
export class BlogService {
  ...
  async create(dto: CreateBlogDto): Promise<Blog> {
    ...
    try {
      const slug = await this.generateUniqueSlug(
        dto.slug || dto.translations[0].title,
      );
  ...
  async addTranslation(
    ...
    try {
      ...
      if (existingTranslation) {
        throw new BadRequestException(
          this.i18n.t('global.blog.TRANSLATION_ALREADY_EXISTS', {
            args: { lang: dto.language },
          }),
        );
      }
      ...
      this.logger.error(
        `Error adding translation for blog ${blogId}: ${error.message}`,
        error.stack,
      );
  ...
  async addOrUpdateTranslation(
    ...
    this.logger.log(
      `Adding/updating translation for blog ${blogId}, language ${translationDto.language}`,
    );
    const blog = await this.findOne(blogId);
    ...
    this.logger.log(
      `Updated translations for blog ${blogId}: ${JSON.stringify(
  ...
  async removeTranslation(
    ...
    if (
      ...
      this.logger.warn(
        `Invalid params: blogId=${blogId}, translationId=${translationId}`,
      );
      throw new BadRequestException(
        this.i18n.t('global.global.INVALID_NUMBER_PARAM', {
          args: { param: 'blogId or translationId' },
        }),
      );
    }
    try {
      ...
      if (!translation) {
        throw new NotFoundException(
          this.i18n.t('global.blog.TRANSLATION_NOT_FOUND'),
        );
      }
      await this.translationRepository.remove(translation);
    } catch (error) {
      this.logger.error(
        `Error removing translation ${translationId} for blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async findOne(id: number): Promise<Blog> {
    ...
    try {
      ...
      this.logger.error(
        `Error fetching blog ${id}: ${error.message}`,
        error.stack,
      );
  ...
  async findBySlug(slug: string, lang?: string): Promise<Blog> {
    ...
    try {
      ...
      if (lang && !blog.translations.some((t) => t.language === lang)) {
        throw new NotFoundException(
          this.i18n.t('global.blog.TRANSLATION_NOT_FOUND', { args: { lang } }),
        );
      }
      return blog;
    } catch (error) {
      this.logger.error(
        `Error fetching blog with slug ${slug}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async update(id: number, dto: UpdateBlogDto): Promise<Blog> {
    ...
    try {
      ...
      this.logger.error(
        `Error updating blog ${id}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async remove(id: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Error deleting blog ${id}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async generateUniqueSlug(slug: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `Error generating slug ${slug}: ${error.message}`,
        error.stack,
      );
      throw new BadRequestException(
        this.i18n.t('global.global.INTERNAL_ERROR'),
  ...
  async getSitemap(lang: string = 'en'): Promise<{ urls: string[] }> {
    ...
    try {
      ...
      const urls = blogs
        .filter((blog) => {
          if (!blog.id || !blog.slug) {
            this.logger.warn(
              `Invalid blog data: id=${blog.id}, slug=${blog.slug}`,
            );
            return false;
          }
          ...
          if (!hasValidTranslation) {
            this.logger.debug(
              `Blog ${blog.slug} has no valid translation for ${lang}`,
            );
          }
      ...
      if (urls.length === 0) {
        this.logger.warn(
          `No valid translations found for blogs in language ${lang}`,
        );
        throw new BadRequestException(
  ...
  async removeTranslationByLanguage(
    ...
    this.logger.debug(
      `Attempting to delete translation for blogId=${blogId}, language=${language}`,
    );
    if (isNaN(blogId) || blogId <= 0) {
      this.logger.warn(`Invalid blogId: ${blogId}`);
      throw new BadRequestException(
    ...
    try {
      ...
      if (!translation) {
        this.logger.warn(
          `Translation not found: blogId=${blogId}, language=${language}`,
        );
        throw new NotFoundException(
          this.i18n.t('global.blog.TRANSLATION_NOT_FOUND', {
            args: { lang: language },
      ...
      await this.translationRepository.remove(translation);
      this.logger.log(
        `Deleted translation for blogId=${blogId}, language=${language}`,
      );
    } catch (error) {
      this.logger.error(
