...
export class BlogController {
  ...
  private validateNumberParam(
    ...
    if (isNaN(num) || num <= 0) {
      ...
      throw new BadRequestException(
        i18n.t('global.global.INVALID_NUMBER_PARAM', {
          args: { param: paramName },
        }),
      );
    }
    return num;
  ...
  private validateStringParam(
    ...
    if (!param || typeof param !== 'string' || param.trim() === '') {
      this.logger.warn(`Invalid ${paramName}: ${param}`);
      throw new BadRequestException(
        i18n.t('global.global.INVALID_PARAM', { args: { param: paramName } }),
      );
    }
    return param;
  }
  ...
  async addOrUpdateTranslation(
    ...
    try {
      return await this.blogService.addOrUpdateTranslation(
        blogId,
        translationDto,
      );
    } catch (error) {
      this.logger.error(
        `Error adding/updating translation for blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async removeTranslationByLanguage(
    ...
    this.logger.log(
      `Removing translation for language ${language} in blog ${blogId}`,
    );
    try {
      await this.blogService.removeTranslationByLanguage(blogId, language);
      return {
        message: i18n.t('global.blog.TRANSLATION_DELETED', {
          args: { lang: language },
        }),
      };
    } catch (error) {
      this.logger.error(
        `Error removing translation for language ${language} in blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async getSitemap(
    ...
    try {
      ...
      return {
        urls: sitemap.urls,
        message: i18n.t('global.global.SITEMAP_GENERATED', {
          args: { count: sitemap.urls.length },
        }),
      };
    } catch (error) {
      this.logger.error(
        `Error fetching sitemap for language ${lang}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException
        ? error
  ...
  async findOne(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async findBySlugAndLang(
    ...
    if (
      ...
      throw new BadRequestException(
        i18n.t('global.global.INVALID_LANGUAGE', {
    ...
    try {
      ...
      this.logger.error(
        `Error fetching blog with slug ${slug} for language ${lang}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(
        `Error updating blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
  ...
  async remove(
    ...
    try {
      ...
      this.logger.error(
        `Error deleting blog ${blogId}: ${error.message}`,
        error.stack,
      );
      throw error instanceof BadRequestException ||
        error instanceof NotFoundException
